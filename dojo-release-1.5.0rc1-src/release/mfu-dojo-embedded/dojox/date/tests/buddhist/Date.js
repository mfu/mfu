if(!dojo._hasResource["dojox.date.tests.buddhist.Date"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource["dojox.date.tests.buddhist.Date"] = true;
...
dojo.requireLocalization("dojo.cldr", "greg", null, ""); 
	...
	[
		{
			...
			setUp: function(){
				...
				dojo.forEach(partLocaleList, function(locale){
					...
					dojo.requireLocalization("dojo.cldr", "buddhist", locale, "ROOT,th");
				});
			},
			runTest: function(t){
			},
